...
now = datetime.now()
spec.name = (now.strftime("%Y-%m-%d %H:%M:%S")) + ‘Planned PowerOn vm %s' % args.vmname

29/04/2021 - Matt Palmer
Changed the vim.Virtual.x object to contain ".PowerOn"
spec.action.name = vim.VirtualMachine.PowerOn

29/04/2021 - Matt Palmer
Because only tasks can only be added individually, I need to find a good way to
...
for i in `cat PO_schedule.txt`; do python scheduled_guestPowerOn.py  -s <vcentre-hostname> -u <first.last@domain> -d 'dd/mm/yyyy HH:MM' -p <passwd> -n $i; done
"""
...
def main():
    ...
    spec.name = (now.strftime("%Y-%m-%d %H:%M:%S")) + ‘Planned PowerOn vm %s' % args.vmname
    #spec.name = ‘Planned PowerOn vm %s' % args.vmname
    spec.description = ''
